...
#define PAGES                                                                  \
  256 // Offset is the remainder and in this case the lower 8 bits. Each hex 'F'
      // represent 4 bits, therefore two F's (FF) represent 8 bits
...
/*Creating a 2-D array to simulate a physical memory, normally a physical memory
 * has pages which have contiguous locations as its part, which is true here as
 * well but instead of initializing an array of size 2^15, constructing it in
 * the form of a 2-D gives us simplicity in working with the array*/
signed char physical_memory[FRAME_COUNT][PAGE_SIZE];

int current_frame_index =
    0; // Using it as a pointer to mark the next available index in our physical
       // memory since it's implemented as a circular array
int frame_page[FRAME_COUNT]; // For inverse mapping to update page_table
signed char *mmapfptr;

// TLB variables
TLBentry TLB[TLB_SIZE];
int TLBindex = 0; // Points to the next position to insert in TLB (similar to
                  // the physical memory logic)
...
int main() {
  ...
  // I have added a safety check in case I have spelling mistakes in my
  // filenames
  if (mmapfile_fd == -1) {
  ...
  mmapfptr =
      mmap(NULL, LOGICAL_ADDRESS_SIZE, PROT_READ, MAP_PRIVATE, mmapfile_fd, 0);
  // END
  ...
  char buff[BUFFER_SIZE];  // To store the opened file's values which will later
                           // be casted as unsigned ints
  int total_addresses = 0; // To check for total addresses in the file
  int TLB_hits =
      0; // Variable to keep track of the number of TLB hits that have occurred
  int page_faults = 0; // Variable to keep track of the number of page faults
                       // that have occurred

  // Loops through the whole addresses.txt file which simulates behaviour of
  // someone requesting memory addresses
  while (fgets(buff, BUFFER_SIZE, fptr) != NULL) {
    ...
    unsigned int page_number =
        logical_address >>
        OFFSET_BITS; // Essentially logical_address / offset_bits in binary
    unsigned int offset =
        logical_address &
        OFFSET_MASK; // Getting the remainder i.e. the lower 8 bits
    ...
    if (frame_number != -1) {
      ...
      {
        frame_number = page_table[page_number]; // We get the frame number from
                                                // the page_table
      } else {
        // A Page fault has occured due to the neither the tlb nor the physical
        // memory having the frame number
        replace_page(page_number);
        frame_number = page_table[page_number]; // Updating the frame number in
                                                // the page_table
        page_faults++;
      }
     ...
     * now we are getting the address from the page number by multiplying the
     * frame number with the page size and adding the offset for that particular
     * page in the logical address that we calculated */
...
// Writes a page from the memory mapping of BACKING_STORE to a frame in physical
// memory
void write_frame(int page_number, int frame_index) {
  // The key here is how memcpy() works in our write_frame() function:
  // 1. The first argument of memcpy() is the destination, which is the
  // physical_memory's
  //    nested array corresponding to the target frame
  //    (physical_memory[frame_index]).
  // 2. The second argument is the source location in the memory-mapped file.
  //    From lab assignments, we learned that to copy specific data from a
  //    memory-mapped file:
  //        memcpy(&intArray[i], mmapfptr + 4 * i, 4);
  //    - This offsets `mmapfptr` by the current position (4 * i) to locate the
  //    next integer
  //      since each integer is 4 bytes.
  // 3. In our case, each signed char takes 1 byte, so for copying an entire
  // page:
  //    - Use `mmapfptr + page_number * PAGE_SIZE` as the source (offset to the
  //    page's location).
  //    - The second argument (`page_number * PAGE_SIZE`) calculates the
  ...
  memcpy(physical_memory[frame_index], mmapfptr + page_number * PAGE_SIZE,
         PAGE_SIZE);
}
...
void replace_page(int page_number) {
  ...
  page_table[page_number] =
      current_frame_index; // Update the page_table as well

  // Update TLB if necessary (if the old page exists and hasn't been replaced in
  // the physical memory, nothing will happen, otherwise new frame will take the
  // position of the existing page number)
